package ru.github.iipolovinkin.testcontainers.kafka;
...
class KafkaContainerClusterTest {
    ...
    public void testKafkaContainerCluster() throws Exception {
        try (
            ...
            Assertions.assertEquals(3, cluster.getBrokers().size());
    ...
    protected void testKafkaFunctionality(String bootstrapServers, int partitions, int rf) throws Exception {
        try (
            ...
            Unreliables.retryUntilTrue(10, TimeUnit.SECONDS, () -> {
                ...
                ConsumerRecord<String, String> next = records.records(topicName).iterator().next();
                Assertions.assertEquals("testcontainers", next.key());
                Assertions.assertEquals("rulezzz", next.value());
                Assertions.assertEquals(topicName, next.topic());
//                assertThat(records)
